...
def __balance_val_split(dataset, val_split=0.0):
    ...
    train_indices, val_indices = train_test_split(
        np.arange(targets.shape[0]), test_size=val_split, stratify=targets
    )
...
def load_data(cfg):
    ...
    tv_normalize = tv.transforms.Normalize(
    ...
    if cfg.dataset == "imagenet":
        ...
        if os.path.isdir(os.path.join(cfg.path, "test")):
            test_set = tv.datasets.ImageFolder(
                root=os.path.join(cfg.path, "test"), transform=val_transform
            )
        ...
        val_transform = tv.transforms.Compose([tv.transforms.ToTensor(), tv_normalize])
