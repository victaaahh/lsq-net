...
def load_checkpoint(model, chkp_file, model_device=None, strict=False, lean=False):
    ...
    if anomalous_keys:
        ...
        if unexpected_keys:
            logger.warning(
                "The loaded checkpoint (%s) contains %d unexpected state keys"
        ...
        if missing_keys:
            for k in missing_keys:
                if k[-5:] != "_fn.s":
                    raise ValueError(
                        "The loaded checkpoint (%s) is missing %d state keys and at least one is not a step size"
    ...
    if lean:
        ...
        logger.info(
            "Loaded checkpoint %s model (next epoch %d) from %s",
            arch,
            start_epoch,
            chkp_file,
        )
