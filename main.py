...
def main():
    ...
    if args.kd.enable:
        ...
        student_model = model
        model = KDModel(teacher_model, student_model, args.kd.freeze_teacher)
    ...
    if args.resume.path and (not args.kd.enable or args.kd.resume_training):
        ...
        args.kd.enable
        and not args.kd.resume_training
        and (args.resume.path or args.kd.teacher_checkpoint)
    ):
        if args.resume.path:
            student_model, _, _ = util.load_checkpoint(
                student_model, args.resume.path, lean=True
            )
